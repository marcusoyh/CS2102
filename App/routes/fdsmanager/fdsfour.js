...
router.post('/', function (req, res, next) {
  ...
  pool.query(query_first, (err, data) => {
    ...
    pool.query(query_one, (err, data2) => {
      ...
      console.log("DATA ROWCOUNT IS" + data.rowCount);
      if (data.rowCount ==0 || data2.rowCount == 0) {
        res.render('fdsmanager/summaryfour', { title: 'Summary Four', month: month, year: year, data: data.rows, totalsalary: 0 });
      } else {
